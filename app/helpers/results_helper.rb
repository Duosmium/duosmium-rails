module ResultsHelper
  ...
  IMAGES_PATH ||= "#{Pathname.new(__dir__)}/../javascript/images/"
  # IMAGES_PATH ||= "/packs/media/images/"
  ...
  def find_bg_color(filename)
    colors = Miro::DominantColors
               .new(("#{IMAGES_PATH}logos/#{find_logo_path(filename).to_s}"))
               .to_hex
  ...
  def name_from_interpreter(i)
    tournament = i.tournament
    out = tournament.start_date.strftime('%Y-%m-%d')
    out += case tournament.level
           ...
           when 'Regionals'
             "_#{tournament.state}_#{tournament.short_name.downcase.split('regional')[0].gsub(" ", "_")}regional"
           else
             "_#{tournament.short_name.downcase.split('invitational')[0].gsub(" ", "_")}invitational"
           end
